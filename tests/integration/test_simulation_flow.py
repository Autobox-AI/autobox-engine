...
from autobox.config.loader import _load_simulation_config as load_simulation_config, _load_metrics_config as load_metrics_config
...
from autobox.schemas.message import Init, SignalMessage, Signal, Ack, Status
from autobox.schemas.planner import PlannerOutput, PlannerStatus, Instruction
...
class TestSimulationFlow:
    ...
    async def test_simple_simulation_flow(
        ...
        worker1_response = Mock(choices=[Mock(message=Mock(content=mock_llm_responses["worker_1"]))])
        worker2_response = Mock(choices=[Mock(message=Mock(content=mock_llm_responses["worker_2"]))])
        mock_worker_llm.return_value.think.side_effect = [worker1_response, worker2_response]
        
        reporter_response = Mock(choices=[Mock(message=Mock(content=mock_llm_responses["reporter"]))])
        ...
        def mock_ask(address, message, timeout=None):
            ...
            if isinstance(message, Init):
                ...
                if message.type == Signal.START:
                    ...
                    if len([m for m in message_log if isinstance(m, SignalMessage) and m.type == Signal.STATUS]) < 3:
                        status = ActorStatus.RUNNING
                    else:
                        status = ActorStatus.COMPLETED
                    return Status(status=status, from_agent="orchestrator", to_agent="simulator")
                elif message.type == Signal.STOP:
                    return Status(status=ActorStatus.STOPPED, from_agent="orchestrator", to_agent="simulator")
        ...
        start_messages = [m for m in message_log if isinstance(m, SignalMessage) and m.type == Signal.START]
        assert len(start_messages) > 0
        
        # Verify status checks happened
        status_messages = [m for m in message_log if isinstance(m, SignalMessage) and m.type == Signal.STATUS]
        assert len(status_messages) > 0
        
        # Verify stop signal was sent
        stop_messages = [m for m in message_log if isinstance(m, SignalMessage) and m.type == Signal.STOP]
        assert len(stop_messages) > 0

    @pytest.mark.asyncio
    @patch("autobox.core.simulator.create_actor")
    @patch("autobox.core.simulator.ActorSystem")
    async def test_simulation_timeout(self, mock_actor_system_class, mock_create_actor, test_config):
        ...
        stop_calls = [
            ...
            if len(call[0]) > 1 and isinstance(call[0][1], SignalMessage) and call[0][1].type == Signal.STOP
        ]
    ...
    @patch("autobox.core.simulator.create_actor")
    @patch("autobox.core.simulator.ActorSystem")
    async def test_simulation_error_handling(self, mock_actor_system_class, mock_create_actor, test_config):
    ...
    @patch("autobox.core.simulator.create_actor")
    @patch("autobox.core.simulator.ActorSystem")
    async def test_simulation_status_transitions(self, mock_actor_system_class, mock_create_actor, test_config):
