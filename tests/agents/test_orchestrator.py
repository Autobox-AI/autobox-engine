...
from thespian.actors import ActorExitRequest, ActorAddress

from autobox.core.agents.orchestrator import Orchestrator
from autobox.schemas.message import (
    SignalMessage, Signal, Message, Ack, Status, Init
)
from autobox.schemas.config import AgentConfig, WorkerConfig, LLMConfig, MailboxConfig
from autobox.schemas.ai import OpenAIModel
from autobox.schemas.actor import ActorStatus, ActorName
from autobox.schemas.config import WorkerConfig, AgentConfig
...
class TestOrchestratorMessageHandling:
    ...
    def test_init_message(self, mock_generate_metrics, mock_create_actor, orchestrator, mock_config, agent_ids):
        ...
        mock_create_actor.side_effect = lambda cls, globalName: mock_actors.get(globalName)
        ...
        assert orchestrator.send.call_count == 6  # planner, evaluator, reporter, 2 workers, and ack to sender
    ...
    def test_start_signal(self, orchestrator):
        ...
        orchestrator.send.assert_any_call(
            orchestrator.planner,
            ANY
        )
        
        # Verify acknowledgment is sent back
        ack_call = [call for call in orchestrator.send.call_args_list 
    ...
    def test_stop_signal(self, orchestrator):
        ...
        # Verify status response is sent
        status_calls = [call for call in orchestrator.send.call_args_list 
